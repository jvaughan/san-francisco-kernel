...
static int cypress_i2c_read(struct i2c_client *client, int reg, u8 * buf, int count)
{
    ...
    if (rc != 1)
    {
        dev_err(&client->dev, "cypress_i2c_read check: read of register %d\n", reg);
        ret = -1;
        goto tp_i2c_rd_exit;
    }
    ...
    if (rc != count)
    {
        dev_err(&client->dev, "cypress_i2c_read check: read %d bytes from reg %d\n", count, reg);
        ret = -1;
    }
...
static int cypress_i2c_write(struct i2c_client *client, int reg, u8 data)
{
    ...
    if (rc != 2)
    {
        dev_err(&client->dev, "cypress_i2c_write check: writing to reg %d\n", reg);
        ret = -1;
    }
    return ret;
}
