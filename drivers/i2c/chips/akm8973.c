...
static int AKI2C_RxData(char *rxData, int length)
{
	...
	if ( rc< 0) {
		pr_err(FYA_TAG"AKI2C_RxData: transfer error,i2c_transfer() return %d\n",rc);
		return -EIO;
	}
...
static int AKI2C_TxData(char *txData, int length)
{
	...
	if (rc < 0) {
		pr_err(KERN_ERR "AKI2C_TxData: transfer error,i2c_transfer() return %d\n",rc);
		return -EIO;
	}
...
static int akm8973_probe(struct i2c_client *client, const struct i2c_device_id * devid)
{
	...
	printk(KERN_ERR "akm8973_probe: compass input devices register OK\n");//add for test
	err = misc_register(&akmd_device);
	if (err) {
		printk(KERN_ERR "akm8973_probe: akmd_device register failed\n");
		goto exit_misc_device_register_failed;
	}
	printk(KERN_ERR "akm8973_probe: akmd_device register OK\n");//add for test
	...
	printk(KERN_ERR "akm8973_probe: akm_aot_device register OK\n");//add for test 
...
exit_alloc_data_failed:
	kfree(akm);
	printk(FYA_TAG
		"In akm8973_probe() ,register device failed or the device do not exist!\n");
