...
static u16 prox_threshold_hi_param = 5000; 
static u16 prox_threshold_lo_param = 4800;
#elif defined(CONFIG_MACH_JOE)
static u16 prox_threshold_hi_param = 5500; 
static u16 prox_threshold_lo_param = 5300;
#else
static u16 prox_threshold_hi_param = 12000; 
static u16 prox_threshold_lo_param = 10000;
#endif
static u8 prox_int_time_param = 0xEE;
static u8 prox_adc_time_param = 0xEE;
static u8 prox_wait_time_param = 0xF2;
...
static irqreturn_t taos_interrupt(int irq, void *data)
{
    u16 status = 0;
    int ret =0;
    disable_irq(taos_datap->pdata->irq);
    if ((ret = (i2c_smbus_write_byte(taos_datap->client, (TAOS_TRITON_CMD_REG | 0x13)))) < 0) {
                printk(KERN_CRIT "TAOS: i2c_write_byte failed in address reg 0x13 in taos interrupt()\n");      
	goto read_reg_fail;
	}	
...
static int __init taos_init(void) {
	...
	printk(KERN_ERR "TAOS: comes before interrupt regist\n");
...
static int taos_ioctl(struct inode *inode, struct file *file, unsigned int cmd, unsigned long arg) {
	...
	switch (cmd) {
		case TAOS_IOCTL_ALS_ON:			
			...
			light_on=1;	
			schedule_delayed_work(&taos_datap->als_work,HZ/5);
		...
		case TAOS_IOCTL_PROX_CALIBRATE:
			...
			taos_cfgp->prox_threshold_hi = ((((prox_max - prox_mean) * 160) + 50)/100) + prox_mean;
			taos_cfgp->prox_threshold_lo = ((((prox_max - prox_mean) * 140) + 50)/100) + prox_mean;
			...
			pr_crit(KERN_ERR "taos prox_cal_threshold_hi=%x,prox_cal_threshold_lo=%x\n",taos_cfgp->prox_threshold_hi,taos_cfgp->prox_threshold_lo);
